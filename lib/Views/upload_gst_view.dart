...
class UploadGSTNoView extends StatefulWidget {
  static const String routeName = '/GSTDetailsView';

  const UploadGSTNoView({Key? key}) : super(key: key);

  @override
  _UploadGSTNoViewState createState() => _UploadGSTNoViewState();
}

class _UploadGSTNoViewState extends State<UploadGSTNoView> {
  ...
  void callGetDetails() {
    ...
    NetworkCalls().callServer(Constants.apiMemberProfile, param).then((value) {
      ...
      GSTNumber.text = _profileModel.MemberProfile![0]!.gstin!;
      setState(() {
        isLoading = true;
        isEnable = false;
      });
  ...
  _onSubmit() {
    if (_panformKey.currentState!.validate()) {
      if (base64Image == '') {
        Get.snackbar('Input Warning', 'Please Pick Proof Image',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Upload GST No'),
      ),
            ...
            child: Column(
              children: [
                ...
                TextFormField(
                  ...
                  maxLines: 1,
                  maxLength: 15,
                  validator: (v) => Validator.gstValidator(v!),
